/*
 * linux/drivers/power/wmt_battery.c
 *
...
static void gpio_ini(void)
{
	...
	if (wmt_getsyspara(varname, buf, &varlen) == 0){                                                                                                                               
        sscanf(buf,"[%x:%x:%x:%x:%x:%x:%x:%x][%x:%x:%x:%x:%x:%x:%x:%x][%x:%x:%x:%x:%x:%x:%x:%x]",              
            ...
            &batlow.ipdaddr);                                                                       
/*	
    printk("Bow: dcin.active = 0x%x  \n",dcin.active);
	printk("Bow: dcin.active data= 0x%x  \n",(dcin.active & 0x01));
	printk("Bow: dcin.active pull= 0x%x  \n",((dcin.active & 0x10)>>4));
	printk("Bow: batstate.active = 0x%x  \n",batstate.active);
	printk("Bow: batstate.active data= 0x%x  \n",(batstate.active & 0x01));
	printk("Bow: batstate.active pull= 0x%x  \n",((batstate.active & 0x10)>>4));
	printk("Bow: batlow.active = 0x%x  \n",batlow.active);
	printk("Bow: batlow.active data= 0x%x  \n",(batlow.active & 0x01));
	printk("Bow: batlow.active pull= 0x%x  \n",((batlow.active & 0x10)>>4));
...
static int __init wmt_batt_init(void)
{
	char buf[128];
...
/*
	    printk(("Bow: wmt_operation = 0x%x \n"),wmt_operation);
	    printk(("Bow: batt_update_time = 0x%x \n"),batt_update_time);
	    printk(("Bow: batt_charge_max = 0x%x \n"),batt_charge_max);
	    printk(("Bow: batt_charge_min = 0x%x \n"),batt_charge_min);
	    printk(("Bow: batt_discharge_max = 0x%x \n"),batt_discharge_max);
	    printk(("Bow: batt_discharge_v9 = 0x%x \n"),batt_discharge_v9);
	    printk(("Bow: batt_discharge_v8 = 0x%x \n"),batt_discharge_v8);
	    printk(("Bow: batt_discharge_v7 = 0x%x \n"),batt_discharge_v7);
	    printk(("Bow: batt_discharge_v6 = 0x%x \n"),batt_discharge_v6);
	    printk(("Bow: batt_discharge_v5 = 0x%x \n"),batt_discharge_v5);
	    printk(("Bow: batt_discharge_v4 = 0x%x \n"),batt_discharge_v4);
	    printk(("Bow: batt_discharge_v3 = 0x%x \n"),batt_discharge_v3);
	    printk(("Bow: batt_discharge_v2 = 0x%x \n"),batt_discharge_v2);
	    printk(("Bow: batt_discharge_v1 = 0x%x \n"),batt_discharge_v1);
	    printk(("Bow: batt_discharge_min = 0x%x \n"),batt_discharge_min);
*/
	return 0;
}
...
static void wmt_bat_update(struct power_supply *bat_ps)
{
	...
	if(batt_operation){
		...
		/*printk("\Bow: bat_low = %d \n",bat_low);*/
		/*printk("\Bow: bat_dcin = %d \n",bat_dcin);*/
		...
		if((bat_low) && (!bat_dcin)){
	        ...
	        if (!ADC_USED){
			/*printk("Bow: Droid book. \n");*/
				/*bat_status = wmt_read_status(bat_ps);*/
				if(bat_dcin){  /* Charging */
				    /*printk("\Bow: bat_status = %d\n",bat_status);*/
					if(bat_status== POWER_SUPPLY_STATUS_DISCHARGING){
						bat_capacity = 100;
					}else{
						bat_capacity = 50;
	...
	/*
	printk("Bow: current_percent = %d \n",current_percent);
    printk("Bow: bat_temp_capacity = %d \n",bat_temp_capacity);
    printk("Bow: capacity %d \n",bat_capacity);
    */
    if((bat_low) ||
	...
	/*
    printk("Bow: ****************************************************************************  \n");
	printk("Bow: bat_low = %d, bat_low_old = %d  \n",bat_low,bat_low_old);
	printk("Bow: bat_dcin = %d, bat_dcin_old = %d  \n",bat_dcin,bat_dcin_old);
	//printk("Bow: bat_health = %d, bat_health_old = %d  \n",bat_health,bat_health_old);
	//printk("Bow: bat_online = %d, bat_online_old = %d  \n",bat_online,bat_online_old);
	printk("Bow: bat_status = %d, bat_status_old = %d  \n",bat_status,bat_status_old);
	printk("Bow: bat_capacity = %d, bat_capacity_old = %d  \n",bat_capacity,bat_capacity_old);
	printk("Bow: first_update = %x  \n",first_update);
	printk("Bow: ****************************************************************************  \n");
   */
	bat_low_old=bat_low;
...
static void wmt_ac_update(struct power_supply *ac_ps)
{
	...
	if(batt_operation){
		...
		ac_dcin_old = ac_dcin;
		/*printk("Bow: ac_dcin = %d, ac_dcin_old = %d \n",ac_dcin,ac_dcin_old);*/
	    if (!first_update) {
			power_supply_changed(ac_ps);
		    ++first_update;
	    }
