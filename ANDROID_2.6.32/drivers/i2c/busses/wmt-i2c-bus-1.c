...
/*++    History :
	...
	2010/03/16 Rename(wmt-i2c-bus.c---->wmt-i2c-bus-1.c)
	           In order to sync file name and i2c port number which describe in datasheet
...
)
{
	...
	for (; ;) {
		...
		*/
		wait_event_result = wait_event_timeout(i2c1_wait, i2c.isr_int_pending ,
			(MAX_RX_TIMEOUT * HZ / 1000)) ;
...
)
{
	...
	for (; ;) {
		...
		*/
		wait_event_result = wait_event_timeout(i2c1_wait, i2c.isr_int_pending , (MAX_TX_TIMEOUT * HZ / 1000)) ;
...
static int i2c_wmt_wait_bus_not_busy(void)
{
	...
	while (1) {
		...
		if (end_time - start_time > 60000) {/*20ms*/
			ret = (-EBUSY) ;
			printk("i2c_err1 : wait but not ready time-out\n\r") ;
			break;
		}
	}
#if 0
	...
	remove_wait_queue(&i2c1_wait, &wait);
#endif
...
)
{
	...
	*/
	...
	if (wakeup) {
		...
		/*
		wake_up_interruptible(&i2c1_wait);
		*/
		wake_up(&i2c1_wait);
	} else
...
static int __init i2c_adap_wmt_init(void)
{
	...
	if (is_master == 1) {
		...
		/**/
		/*
		*(volatile unsigned int *)PMC_ClOCK_ENABLE_LOWER |= (BIT9);
		*/
		auto_pll_divisor(DEV_I2C1, SET_DIV, 2, 20);/*20M Hz*/
		GPIO_CTRL_GP9_VSYNC_BYTE_VAL &= ~(BIT4 | BIT5);
		GPIO_PULL_EN_GP9_VSYNC_BYTE_VAL |= (BIT4 | BIT5);
		GPIO_PULL_CTRL_GP9_VSYNC_BYTE_VAL |= (BIT4 | BIT5);
		/*
