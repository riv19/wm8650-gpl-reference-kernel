...
void disp_patch(vpp_tvsys_t tvsys,vpp_tvconn_t tvconn)
{
	if( tvsys == VPP_TVSYS_NTSC ){
		switch( tvconn ){
			case VPP_TVCONN_CVBS:
			case VPP_TVCONN_SVIDEO:
				vppif_reg32_out(DISP_BASE_ADDR+0x1c,0x0252506c);
				...
				vppif_reg32_out(DISP_BASE_ADDR+0x44,0x3d);
				...
				vppif_reg32_out(DISP_BASE_ADDR+0x10,0xA0);
				break;
	...
	if( tvsys == VPP_TVSYS_PAL ){
		switch( tvconn ){
			...
			case VPP_TVCONN_SVIDEO:
				vppif_reg32_out(REG_DISP_DACMOD,0x01c70c00);
				vppif_reg32_out(DISP_BASE_ADDR+0x1c,0x02555579);
				vppif_reg32_out(DISP_BASE_ADDR+0x2c,0x0);
...
void disp_suspend(int sts)
{
	switch( sts ){
		...
		case 2:	// backup register
			...
			disp_reg_bk2 = vpp_backup_reg(DISP_BASE2_ADDR+0x00,0x54); /* 0x00 - 0x54 */
			break;
		default:
			break;
	}
...
void disp_resume(int sts)
{
	switch( sts ){
		case 0:	// restore register
			...
			vpp_restore_reg(DISP_BASE2_ADDR+0x00,0x54,disp_reg_bk2); /* 0x00 - 0x54 */
			disp_reg_bk2 = 0;
			p_disp->reg_bk = 0;			
			break;
