...
{
	/*
	alpha enable : 1-alway show, 0-check alpha byte
	alpha byte : 0-check colkey, other-show
	colkey enable (inverse 0): show pixel != colkey
				  (inverse 1): show pixel == colkey
	*/
	...
	vppif_reg32_write(GOVRH_CUR_COLKEY_INVERT,0);
...
int govrh_irqproc_set_position(void *arg)
{
	...
	if( (begin + p_cursor->fb_p->fb.img_w) > p_govrh->fb_p->fb.img_w ){ // over right edge
		width = p_govrh->fb_p->fb.img_w - begin;
	}
...
//		p_cursor->posx,p_cursor->hotspot_x,pos_x,crop_x,width);
	...
	if( (begin + p_cursor->fb_p->fb.img_h) > p_govrh->fb_p->fb.img_h ){ // over right edge
		height = p_govrh->fb_p->fb.img_h - begin;
	}
...
void govrh_CUR_set_framebuffer(vdo_framebuf_t *fb)
{
	...
	p_cursor->posx = 0;
	p_cursor->posy = 0;
	p_cursor->hotspot_x = 0;
	p_cursor->hotspot_y = 0;
	...
	vpp_cache_sync();
	govrh_CUR_set_color_key(0,0,0x0);
	govrh_CUR_set_colfmt(p_govrh->fb_p->fb.col_fmt);
}
