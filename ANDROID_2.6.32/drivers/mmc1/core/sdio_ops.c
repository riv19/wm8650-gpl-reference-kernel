...
int mmc1_send_io_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
...
//	cmd.flags = MMC_RSP_SPI_R4 | MMC_RSP_R4 | MMC_CMD_BCR;
	...
	if (rocr)
		*rocr = cmd.resp[mmc1_host_is_spi(host) ? 1 : 0];

	DBG("[%s] e\n",__func__);
	return err;
}

int mmc1_io_rw_direct(struct mmc_card *card, int write, unsigned fn,
...
int mmc1_io_rw_extended(struct mmc_card *card, int write, unsigned fn,
