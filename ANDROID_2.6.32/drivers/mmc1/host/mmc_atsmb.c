/*++
linux/drivers/mmc1/host/mmc_atsmb.c
...
static unsigned int fmax1 = 515633;
/*2008/10/6 RichardHsu-s*/
...
atsmb1_request_end(struct atsmb_host *host, struct mmc_request *mrq)
{
	...
	/*DBG("100");*/
	mmc1_request_done(host->mmc, mrq);
	/*DBG("101\n");*/
	spin_lock(&host->lock);
	DBG("[%s] e\n",__func__);
}
...
//		card_addr += total_dma_len>>(mmc_card_blockaddr(host->mmc->card_selected) ? 9 : 0);
		card_addr += total_dma_len>>(mmc1_card_blockaddr(host->mmc->card) ? 9 : 0);	//zhf: modified by James Tian
...
#if 0
        /* If card is not in slot, do not invoke mmc_rescan */
		if (status_0 & ATSMB_CARD_NOT_IN_SLOT_GPI)
			return IRQ_HANDLED;
#endif
...
{
	struct atsmb_host *host = mmc1_priv(mmc);
	unsigned char status_0 = 0;
	unsigned long flags;
	unsigned long ret = 0;
	DBG("[%s] s\n",__func__);
...
static void atsmb1_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_OFF) {
		...
		if (MMC_DRIVER_VERSION == MMC_DRV_3465) {       /* 3429 power up setting */
			...
			GPIO_PULL_CTRL_GP3_STORGE_BYTE_VAL|= GPIO_SD1_CD;
			...
			GPIO_PULL_EN_GP3_STORGE_BYTE_VAL |= GPIO_SD1_CD;
			...
			*ATSMB_BUS_MODE |= ATSMB_CST;
            ...
            GPIO_CTRL_GP15_NAND_7_0_BYTE_VAL &= ~GPIO_SD1_Data;
			GPIO_CTRL_GP29_NAND_CFG_1_BYTE_VAL &= ~GPIO_SD1_WriteProtect;
			GPIO_CTRL_GP3_STORGE_BYTE_VAL &= ~GPIO_SD1_CD;           
...
static int __init atsmb1_probe(struct platform_device *pdev)
{
	...
	mmc_host = mmc1_alloc_host(sizeof(struct atsmb_host), dev);
	if (!mmc_host) {
		ret = -ENOMEM;
	...
	*ATSMB_INT_MASK_0 |= 0x80; /*or 0x40?*/
	...
	printk(KERN_INFO "WMT ATSMB1 (AHB To SD/MMC Bus) controller registered!\n");
	mmc1_add_host(mmc_host);
	//TODO: !!!!
    GPIO_CTRL_GP3_STORGE_BYTE_VAL 	&= ~GPIO_SD1_CD;
...
static int atsmb1_remove(struct platform_device *pdev)
{
	...
	*ATSMB_SD_STS_1 |= 0xff;
	...
	(void)mmc1_free_host(mmc_host);/* also free atsmb_host.*/
	DBG("[%s] e2\n",__func__);
	return 0;
}
...
static int atsmb1_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (mmc) {
		/*struct atsmb_host *host = mmc_priv(mmc);*/
		ret = mmc1_suspend_host(mmc, state);
		if (ret == 0) {
			/*disable all interrupt and clear status by resetting controller. */
			*ATSMB_BUS_MODE |= ATSMB_SFTRST;
			*ATSMB_BLK_LEN &= ~(0xa000);
...
static int atsmb1_resume(struct platform_device *pdev)
{
	...
	if (mmc) {
		...
		*ATSMB_INT_MASK_0 |= 0x80;	/* or 40?*/
		...
		if ((PMCWS_VAL & BIT14) && !mmc->bus_dead) {
...
static int __init atsmb1_init(void)
{
	...
	ret = platform_driver_probe(&atsmb_driver, atsmb1_probe);
