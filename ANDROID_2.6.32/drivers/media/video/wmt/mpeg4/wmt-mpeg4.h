...
*-----------------------------------------------------------------------------*/
/*----------------- MODULE DEPENDENCY ----------------------------------------*/
#ifndef WM865_MPEG4_H
#define WM865_MPEG4_H
...
typedef struct {
  ...
  mpeg4_attr_t       attr;    /* Come from user space for HW decoder */
  mpeg4_input_t      arg_in;  /* Come from user space */
  mpeg4_frameinfo_t  arg_out; /* Return to user space later */
  mpeg4_capability_t *capab_tbl;

  /* Following members are used for hw-mpeg4.c only */
  mpeg4_status       _status;
#ifdef __KERNEL__
  spinlock_t         _lock;
#endif
} mpeg4_drvinfo_t;
...
/*------------------------------------------------------------------------------
   All MPEG4 HW decoder should export and implement follwing APIs
------------------------------------------------------------------------------*/
...
int wmt_mpeg4_isr(int irq, void *dev_id, struct pt_regs *regs);
int wmt_mpeg4_open(mpeg4_drvinfo_t *drv);
int wmt_mpeg4_close(void);
int wmt_mpeg4_probe(void);
int wmt_mpeg4_suspend(void);
int wmt_mpeg4_resume(void);
