...
//	if(((ep->bEndpointAddress & 0x7F) == 1) && (req->req.length == 0x18))
//		printk(KERN_INFO "d1");
...
//		printk(KERN_INFO "n %x ", length); //gri				
////    printk(KERN_INFO "n_i_d %4.4x %4.4x %4.4x\n", length, req->req.length,req->req.actual); //gri
...
//    printk(KERN_INFO "d %3.3x %3.3x\n", length, req->req.length); //gri		
...
//			}			
	...
	//if ((req != 0) && (empty_data == 0) && (ep->bEndpointAddress == 0))
...
/*		*(volatile unsigned int *)0xd8130254 &= ~0x00000080;*/   /*DPM needed*/
	*(volatile unsigned char*)0xd8007A49 |= 0x04;
...
/*		*(volatile unsigned int *)0xd8130254 |= 0x00000080;*/      /*DPM needed*/
	if ((*(volatile unsigned char *)0xd8007A49)&0x04) {
		*(volatile unsigned char*)0xd8007A49 &= ~0x04;
		mdelay(1);
	}
