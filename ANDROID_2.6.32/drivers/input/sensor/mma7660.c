...
static int get_axisset(void)
{
	...
	if (wmt_getsyspara("wmt.io.gsensor", varbuf, &varlen)) {
		printk(KERN_DEBUG "wmt.io.gsensor not defined!\n");
		return 0;
	} else {
		...
		if (n != 8) {
			printk(KERN_ERR "wmt.io.gsensor format is incorrect!\n");
			return 0;
		}
		printk(KERN_DEBUG "wmt.io.gsensor = %d:%d:%d:%d:%d:%d:%d:%d\n",
			l_sensorconfig.op,
			l_sensorconfig.samp,
			l_sensorconfig.xyz_axis[0][0],
			l_sensorconfig.xyz_axis[0][1],
...
static int __init mma7660_init(void)
{
	...
	if (!ret || !l_sensorconfig.op) {
		printk(KERN_INFO "G-Sensor is disabled\n");
		return -EINVAL;
	}
